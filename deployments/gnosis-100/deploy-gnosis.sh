...
# Replaces the bare `$(... 2>&1)` capture pattern that was hiding failures
# (any non-zero exit got swallowed by the variable assignment, killing the
# script under set -e with no output to indicate which step failed).
#
...
# Verify the createRollup tx actually succeeded (status 1) — cast send returns 0
